...
# ---------- Theme & Rounded Styling ----------
def style_gui(root):
    root.configure(bg="#0E1A2A")
...
def animate_roll():
    for _ in range(8):
        ...
        img = ImageTk.PhotoImage(Image.open(f"dice{face}.png").resize((120, 120)))
...
def simulate():
    ...
    # ANIMATION FOR RESULT BOXES (1 sec total delay before graph)
    for i in range(1, 7):
        root.after(i * 120, lambda i=i: create_result_box(i, results, sample_space))

    # SHOW GRAPH **AFTER 1 SECOND**
    root.after(2000, lambda: show_graph(results, sample_space))

# ---------- Create Animated Result Boxes ----------
def create_result_box(i, results, sample_space):
    prob = results.count(i) / sample_space
    text = f"Face {i}:"
    value = f"{prob * 100:.2f}%"

    frame = tk.Frame(results_container, bg="#1E2E3E", padx=6, pady=4)
    frame.pack(pady=5, anchor="center")  # CENTERED

    bold_label = tk.Label(frame, text=text, font=("Inter", 12, "bold"),
                          bg="#1E2E3E", fg="white")
    bold_label.pack(side="left", padx=2)

    val_label = tk.Label(frame, text=value, font=("Poppins", 12),
                         bg="#1E2E3E", fg="white")
...
def fade_in(widget):
    try:
        for alpha in range(0, 11):
            widget.update()
            widget.attributes("-alpha", alpha/10)  # fade effect
            time.sleep(0.3)
    except:
        pass  # avoids crash on Windows

# ------------ Graph -------------
def show_graph(results, sample_space):
    for widget in graph_frame.winfo_children():
        widget.destroy()

    # Zoomable graph only when sample space <= 1000
    if sample_space <= 1000:
        x_vals = list(range(1, sample_space + 1))
        fig = plt.Figure(figsize=(6.3, 4), dpi=120, facecolor="#0E1A2A")
        ax = fig.add_subplot(111)
        ax.set_facecolor("#1E2E3E")

        bright_colors = ["#E95E3F", "#E3CE59", "#63B163", "#408ECF", "#FF9FFF", "#89F8F8"]

        for i in range(1, 7):
            prob_values = [results[:x].count(i) / x for x in x_vals]
            ax.plot(x_vals, prob_values, label=f"Face {i}", linewidth=0.8, color=bright_colors[i-1])

        ax.grid(True, linestyle="--", alpha=0.5)
        ax.set_title("Probability vs Rolls", color="#D9D9D9", fontsize=12, fontweight="bold")
        ax.set_xlabel("Roll Count", color="white", fontsize=10)
...
# ---------- GUI Layout ----------
root = tk.Tk()
root.title("🎲Dice Simulator with Stats")
root.geometry("650x780")
style_gui(root)

ttk.Label(root, text="🎲 Single Dice Simulator", font=TITLE_FONT).pack(pady=12)

frame = ttk.Frame(root, style="TFrame")
frame.pack(pady=5)

ttk.Label(frame, text="Number of Rolls:").grid(row=0, column=0, padx=6)
entry = ttk.Entry(frame, width=12, font=ENTRY_FONT, style="Rounded.TEntry")
entry.grid(row=0, column=1, padx=4)

anim_label = ttk.Label(root)
anim_label.pack(pady=10)

run_btn = ttk.Button(root, text="Roll Dice!", command=simulate, style="Rounded.TButton")
run_btn.pack(pady=12)

ttk.Label(root, text="Results:").pack()

results_container = tk.Frame(root, bg="#0E1A2A")
results_container.pack(fill="x", pady=5)

graph_frame = tk.Frame(root, bg="#0E1A2A")
